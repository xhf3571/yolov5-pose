...
def test(data,
    ...
    s = ('%20s' + '%12s' * 10) % ('Class', 'Images', 'Labels', 'P', 'R', 'mAP@.5', 'mAP@.75', 'mAP@.5:.95', 'APE', 'APM', 'APL')
    ...
    if len(stats) and stats[0].any():
        ...
        nt = torch.zeros(1)
        mp, mr, map50, map75, map, ape, apm, apl = 0., 0., 0., 0., 0., 0., 0., 0.
    ...
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(ap_class):
            print(pf % (names[c], seen, nt[c], p[i], r[i], ap50[i], ap75[i], ap[i], 0, 0, 0))  # No per-class size metrics
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    return (mp, mr, map50, map75, map, ape, apm, apl, *(loss.cpu() / len(dataloader)).tolist()), maps, t


if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='test.py')
    parser.add_argument('--weights', nargs='+', type=str, default='/home/disk/xuhaifeng/yolov5-pose/runs/train/yolov5s_kpts-crowdpose/weights/best.pt', help='model.pt path(s)')
    parser.add_argument('--data', type=str, default='data/crowdpose.yaml', help='*.data path')
    ...
    parser.add_argument('--kpt-label', default=True, action='store_true', help='Whether kpt-label is enabled or not')
